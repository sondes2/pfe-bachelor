            ...
            <div class="m-portlet__head-tools">
                <ul class="m-portlet__nav">
                    <li class="m-portlet__nav-item">
                        <a href="{{Route('Challengs.create')}}" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                            <span>
													<i class="flaticon-add-circular-button"></i>
													<span>Add a new Challenge</span>
                            </span>
                        </a>
                    </li>
                </ul>
...
var DatatableAutoColumnHideDemo= {
    init:function() {
        $(".m_datatable").mDatatable( {
            ...
            , columns:[ {
                field:"title", title:"Title", width:150, template:function(t) {
                    return '<div class="m-card-user m-card-user--sm">'+
                                    '<div class="m-card-user__pic">'+
                                        '<img src="images/events/'+t.image+'.jpg" class="m--img-rounded m--marginless" alt="photo">'+
                                    '</div>'+
                                    '<div class="m-card-user__details">'+
                                        '<span class="m-card-user__name">'+t.title +'</span>'+
                                    '</div>'+
                                '</div>';
                }
            ...
            , {
                field: "startDate", title: "Date"
            },  {
                field: "id", title: "Actions", template:function(t) {
                    return '<a href="/Challengs/'+t.id+'/edit" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Edit">'+
                          '<i class="la la-edit"></i>'+
                        '</a>'+
                        '<form method="post" action="/Challengs/'+t.id+'" style="display: inline-block;">'+
                        '<input type="hidden" name="_method" value="DELETE">'+
                        '<button type="submit" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"><i class="la la-trash"></i></button></form>';
                }
            }
            ]
        }
